...
$querySiswa  = mysqli_query($koneksi, "SELECT COUNT(*) AS total_siswa FROM tb_user WHERE role = 2");
$rowSiswa     = mysqli_fetch_assoc($querySiswa);
$totalSiswa   = $rowSiswa['total_siswa'];

$queryLulus = mysqli_query($koneksi, "SELECT COUNT(*) AS status_siswa FROM tb_nilai WHERE status = 'LULUS'");
$rowLulus   = mysqli_fetch_assoc($queryLulus);
$siswaLulus = $rowLulus['status_siswa'];

$queryGagal = mysqli_query($koneksi, "SELECT COUNT(*) AS status_siswa FROM tb_nilai WHERE status = 'GAGAL'");
$rowGagal   = mysqli_fetch_assoc($queryGagal);
$siswaGagal = $rowGagal['status_siswa'];

$persentaseLulus = ($siswaLulus / $totalSiswa) * 100;

$persentaseGagal = ($siswaGagal / $totalSiswa) * 100;

$totalUjian = $siswaLulus + $siswaGagal; // Siswa yang sudah mengikuti ujian

// Menghitung persentase siswa lulus
$persentaseLulus = ($totalSiswa > 0) ? ($siswaLulus / $totalSiswa) * 100 : 0;

// Menghitung persentase siswa gagal
$persentaseGagal = ($totalSiswa > 0) ? ($siswaGagal / $totalSiswa) * 100 : 0;

// Menghitung persentase siswa yang sudah mengikuti ujian
$persentaseUjian = ($totalSiswa > 0) ? ($totalUjian / $totalSiswa) * 100 : 0;
  ...
  <div class="content-wrapper">
    ...
    <div class="row">
      <div class="col-md-3 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <p class="card-title text-md-center text-xl-left">Jumlah Siswa</p>
            <div class="d-flex justify-content-between align-items-center">
              <h3 class="mb-0 mb-md-2 mb-xl-0"><?= $totalSiswa ?> Siswa</h3>
              <i class="ti-user icon-md text-muted mb-0 mb-md-3 mb-xl-0"></i>
            </div>
            <!-- <p class="mb-0 mt-2 text-danger">20<span class="text-black ms-1"><small>(30 days)</small></span></p> -->
          </div>
        </div>
      </div>
      <div class="col-md-3 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <p class="card-title text-md-center text-xl-left">siswa lulus ujian</p>
            <div class="d-flex flex-wrap justify-content-between justify-content-md-center justify-content-xl-between align-items-center">
              <h3 class="mb-0 mb-md-2 mb-xl-0 order-md-1 order-xl-0"><?=$rowLulus['status_siswa'] ?> Siswa</h3>
              <i class="ti-face-smile icon-md text-muted mb-0 mb-md-3 mb-xl-0"></i>
            </div>
            <p class="mb-0 mt-2 text-success text-md-center" ><?= number_format($persentaseLulus, 2) ?>% <span class="text-black ms-1"><small>
                  <!-- (30 days) -->
                </small></span></p>
          </div>
        </div>
      ...
      <div class="col-md-3 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <p class="card-title text-md-center text-xl-left">Siswa gagal ujian</p>
            <div class="d-flex flex-wrap justify-content-between justify-content-md-center justify-content-xl-between align-items-center">
              <h3 class="mb-0 mb-md-2 mb-xl-0 order-md-1 order-xl-0"><?= $rowGagal['status_siswa'] ?> Siswa</h3>
              <i class="ti-face-sad icon-md text-muted mb-0 mb-md-3 mb-xl-0"></i>
            </div>
            <p class="mb-0 mt-2 text-danger text-md-center"><?= number_format($persentaseGagal, 2) ?>%<span class="text-black ms-1"><small></small></span></p>
          </div>
        </div>
      </div>
      <div class="col-md-3 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            <p class="card-title text-md-center text-xl-left">Siswa yang telah ujian</p>
            <div class="d-flex flex-wrap justify-content-between justify-content-md-center justify-content-xl-between align-items-center">
              <h3 class="mb-0 mb-md-2 mb-xl-0 order-md-1 order-xl-0"><?= $totalUjian ?> Siswa</h3>
              <i class="ti-pencil-alt icon-md text-muted mb-0 mb-md-3 mb-xl-0"></i>
            </div>
            <p class="mb-0 mt-2 text-success text-md-center"><?= number_format($persentaseUjian, 2) ?>%<span class="text-black ms-1"></span></p>
          </div>
        </div>
      </div>
    </div>
